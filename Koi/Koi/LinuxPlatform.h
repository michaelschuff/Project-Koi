...
namespace koi {
    ...
    public:
        ...
        virtual koi::rcode SetWindowTitle(const std::string& s) override { X11::XStoreName(koi_Display, koi_Window, s.c_str()); return koi::OK; }
        ...
        virtual koi::rcode HandleSystemEvent() override {
            ...
            while (XPending(koi_Display)) {
                ...
                } else if (xev.type == MotionNotify) { ptrPGE->koi_UpdateMouse(xev.xmotion.x, xev.xmotion.y); }
                else if (xev.type == FocusIn) { ptrPGE->koi_UpdateKeyFocus(true); }
                else if (xev.type == FocusOut) { ptrPGE->koi_UpdateKeyFocus(false); }
                else if (xev.type == ClientMessage) { ptrPGE->koi_Terminate(); }
            }
            return koi::OK;
        }
    };
